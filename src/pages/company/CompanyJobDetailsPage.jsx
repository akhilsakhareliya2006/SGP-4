...
function CompanyJobDetailsPage() {
  ...
  /* ---------- FETCH JOB DETAILS ---------- */
  /* ---------- FETCH JOB DETAILS ---------- */
  const fetchJobDetails = useCallback(async (signal) => { // 👈 1. Add signal parameter
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      if (err.name === 'AbortError') return; // 👈 3. Ignore intentional aborts
      console.error("Job details fetch error:", err);
      setError(err.message);
    } finally {
      if (!signal || !signal.aborted) { // 👈 4. Guard the loading state
        setIsLoading(false);
      }
    }
  }, [apiUrl, id]);
  ...
  useEffect(() => {
    ...
    return () => controller.abort(); // 👈 6. Cleanup on unmount
  }, [fetchJobDetails]);
// --- Safe Variable Assignments ---
  const isExpired = job ? new Date(job.dueDate) < new Date() : false;
    ...
    <div className="employees-page" style={{ height: "100%", overflowY: "auto", paddingBottom: "80px" }}>
      ...
      <div className="employees-header" style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "2rem" }}>
        ...
        <div style={{ flexGrow: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h2 className="page-title">{job?.title || "Loading Job..."}</h2>
            <p className="page-subtitle" style={{ fontFamily: 'monospace', color: '#64748b' }}>
              Job ID: {job?.id ? job.id.slice(0, 8).toUpperCase() : "........"}
            </p>
          </div>
          {job && (
